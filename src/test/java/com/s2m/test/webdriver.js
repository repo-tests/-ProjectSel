...
options.header =
    "package ${packageName};\n" +
        ...
        indents(1) + "private Recorder recorder;\n" +
        indents(0) + "\n" +
        indents(1) + "@Before\n" +
...
options.footer =
    indents(1) + "}\n" +
        ...
        indents(2) + "recorder.stopRecording();\n" +
        ...
        indents(1) + "}\n" +
        indents(0) + "\n" +
        indents(1) + "private  WebElement findElement( final By locator) {\n" +
        indents(2) + "(new WebDriverWait(driver, 30,1000)).until(ExpectedConditions.presenceOfElementLocated(locator));\n" +
        indents(3) + "(new WebDriverWait(driver, 30,1000)).until(ExpectedConditions.visibilityOfElementLocated(locator));\n" +
        indents(3) + "WebElement element=driver.findElement(locator);\n" +
        indents(3) + "if (element.isEnabled()==false )  ((JavascriptExecutor) driver).executeScript(\"arguments[0].disabled = false\", element);\n" +
        indents(3) + "return element;\n" +
        indents(1) + "}\n" +
        indents(0) + "\n" +
        indents(1) + "public  void optionalClick( final By locator) {\n" +
        indents(2) + "WebElement we=null;\n" +
        indents(3) + "try{\n" +
        indents(3) + "we = driver.findElement( locator );\n" +
        indents(3) + "we.click();\n" +
        indents(3) + "} catch ( StaleElementReferenceException ser ) {\n" +
        indents(1) + "\n" +
        indents(3) + "} catch ( NoSuchElementException nse ) {\n" +
        indents(1) + "\n" +
        indents(3) + "} catch ( Exception e ) {\n" +
        indents(0) + "}\n" +
        indents(0) + "\n" +
        indents(0) + "}\n" +
        indents(0) + "\n" +
...
WDAPI.Driver.prototype.findElement = function(locatorType, locator) {
  return new WDAPI.Element("findElement(" + WDAPI.Driver.searchContext(locatorType, locator) + ")");
};
...
WDAPI.Element.prototype.clear = function() {
  return  "Thread.sleep(1000); "+ this.ref + ".clear()";
};

WDAPI.Element.prototype.click = function() {
	if (this.ref.indexOf("option[value")>-1) {return "";}
	else { return this.ref + ".click()";}
};
